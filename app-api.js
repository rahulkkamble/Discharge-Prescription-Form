...
export default function App() {
  ...
  // Add this at the top inside App function
  ...
  // Above: other useState hooks
  const [patients, setPatients] = useState([]);
  const [practitioners, setPractitioners] = useState([]);
  const [selectedPatient, setSelectedPatient] = useState(null);
  const [selectedPractitioner, setSelectedPractitioner] = useState(null);
  // Below: your existing practitioner/composition state continues as before

  useEffect(() => {
    async function fetchMockLists() {
      const patientsData = await fetch("/patients.json").then((res) => res.json());
      const practitionersData = await fetch("/practitioners.json").then((res) => res.json());
      ...
      if (practitionersData.length > 0) {
        const spre = practitionersData[0]; // first practitioner
        setSelectedPractitioner(spre);

        // Convert FHIR name array → "Given Family"
        const practitionerName =
          (spre.name?.[0]?.given ? spre.name[0].given.join(" ") : "") +
          (spre.name?.[0]?.family ? " " + spre.name[0].family : "");

        setPractitioner({
          name: practitionerName.trim(),
          license: spre.identifier?.[0]?.value || "",
        });
      }

      if (patientsData.length > 0) {
        const sel = patientsData[0]; // first patient
        setSelectedPatient(sel);

        const patientName =
          (sel.name?.[0]?.given ? sel.name[0].given.join(" ") : "") +
          (sel.name?.[0]?.family ? " " + sel.name[0].family : "");
      ...
      if (patientsData.length > 0) {
        const sel = patientsData[0];
        setSelectedPatient(sel);
        // map FHIR-like patient to your simple form shape
        setPatient({
          name:
            (sel.name?.[0]?.given ? sel.name[0].given.join(" ") : "") +
            (sel.name?.[0]?.family ? " " + sel.name[0].family : ""),
      ...
      if (practitionersData.length > 0) {
        const spre = practitionersData[0];
        setSelectedPractitioner(spre);
        setPractitioner({
          name:
            (spre.name?.[0]?.given ? spre.name[0].given.join(" ") : "") +
  ...
  */
  const buildBundle = () => {
    ...
    /* Patient */
    const patientResource = selectedPatient;

    /* Practitioner */
    const practitionerResource = selectedPractitioner;
    ...
    const medicationResources = medications.map((m, idx) => ({
      ...
      subject: {
        reference: `urn:uuid:${patientId}`, display: selectedPatient || {}
          .name
      },
      authoredOn: formatDateOnly(composition.date),
      requester: {
        reference: `urn:uuid:${practitionerId}`, display: selectedPractitioner || {}
          .name
      },
  ...
  return (
    <div className="container py-4">
        ...
        <div className="card-body">
          <div className="row g-2">
            <div className="col-md-6">
              ...
              <input name="name" type="text" className="form-control" value={selectedPractitioner || {}
                .name} onChange={handlePractitionerChange} />
            </div>
            <div className="col-md-6">
              <label className="form-label">Medical License No. <span className="text-danger">*</span></label>
              <input name="license" type="text" className="form-control" value={selectedPractitioner || {}
                .license} onChange={handlePractitionerChange} />
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="card-body">
          <div className="row g-2">
            <div className="col-md-4">
              ...
              <input name="name" type="text" className="form-control" value={selectedPatient || {}
                .name} onChange={handlePatientChange} />
            </div>
            <div className="col-md-4">
              <label className="form-label">Medical Record No. <span className="text-danger">*</span></label>
              <input name="mrn" type="text" className="form-control" value={selectedPatient || {}
                .mrn} onChange={handlePatientChange} />
            </div>
            <div className="col-md-4">
              <label className="form-label">Phone</label>
              <input name="phone" type="tel" className="form-control" value={selectedPatient || {}
                .phone} onChange={handlePatientChange} />
            </div>

            <div className="col-md-4 mt-2">
              <label className="form-label">Date of Birth <span className="text-danger">*</span></label>
              <input name="birthDate" type="date" className="form-control" value={selectedPatient || {}
                .birthDate} onChange={handlePatientChange} />
            </div>
            <div className="col-md-4 mt-2">
              <label className="form-label">Gender <span className="text-danger">*</span></label>
              <select name="gender" className="form-select" value={selectedPatient || {}
                .gender} onChange={handlePatientChange}>
      ...
      {/* Actions */}
      <div className="mb-4">
        <button className="btn btn-primary me-2" onClick={handleGenerate}>Generate JSON</button>
        <button className="btn btn-outline-primary me-2" onClick={handleDownload}>Download JSON</button>
        <button className="btn btn-outline-secondary" onClick={handleCompare}>Compare With Example</button>
      ...
      {/* Compare Modal (simple line diff) */}
      {showCompareModal && (
          ...
          <div className="modal-dialog modal-xl" role="document">
            <div className="modal-content">
              <div className="modal-header">
                ...
                <button type="button" className="btn-close" aria-label="Close" onClick={() => setShowCompareModal(false)}></button>
              </div>
                ...
                <div className="row">
                  <div className="col-md-6 border-end">
                    ...
                    <div style={{ maxHeight: "60vh", overflow: "auto" }}>
                      {diffRows.map((r, i) => (
                        <div key={i} style={{ whiteSpace: "pre-wrap", background: r.same ? "transparent" : "#fff0f0" }}>{r.leftLine}</div>
                      ))}
                    </div>
                  </div>
                  <div className="col-md-6">
                    ...
                    <div style={{ maxHeight: "60vh", overflow: "auto" }}>
                      {diffRows.map((r, i) => (
                        <div key={i} style={{ whiteSpace: "pre-wrap", background: r.same ? "transparent" : "#f0fff0" }}>{r.rightLine}</div>
                      ))}
                    </div>
                  </div>
                </div>
                <div className="mt-3"><small className="text-muted">Red highlights differences in example; green highlights differences in generated JSON.</small></div>
              </div>
